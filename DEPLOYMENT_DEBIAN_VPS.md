...
### 8.2 Verify Certificate Installation
```bash
sudo certbot certificates
```

### 8.3 Auto-Renewal Setup (Automatic)
...
### 9.1 Edit PHP-FPM Configuration
```bash
...
### 9.2 Edit PHP-FPM Pool Configuration
```bash
...
### 10.2 Set Correct File Permissions
```bash
sudo chown -R www-data:www-data /var/www/muhaseb-pro
...
## Step 12: Setup Logging and Monitoring

### 12.1 Check Application Logs
```bash
tail -f /var/www/muhaseb-pro/storage/logs/laravel.log
```

### 12.2 Check Nginx Logs
```bash
# Access logs
...
### 12.4 Setup Log Rotation (Optional)
```bash
...
## Step 13: Setup Cron Jobs for Laravel Scheduler (Optional)
...
## Step 14: Setup MySQL Backups (Recommended)
...
### 14.2 Make Executable and Setup Cron
```bash
